...
object IP {
  ...
  implicit val ipEncoder: JsonEncoder[IP] =
    ...
    .contramap(ip =>
      IPinternal(ip.ip, ip.country_code2, ip.country_code3, ip.country_name, ip.state_prov, ip.district, ip.city,
        ip.zipcode, ip.latitude, ip.longitude, ip.organization
      ))
}
